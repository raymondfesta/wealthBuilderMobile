...
struct SpendingPatternAnalyzer {
    ...
    static func generateBudgetsFromHistory(
        ...
        print("📊 [Budget Generation] Starting budget generation from \(transactions.count) total transactions")
        ...
        print("📊 [Budget Generation] Filtered to \(recentTransactions.count) transactions from last \(months) months (cutoff: \(cutoffDate))")
        ...
        print("📊 [Budget Generation] Found \(categorySpending.keys.count) expense categories")
        for (category, amounts) in categorySpending {
            let total = amounts.reduce(0, +)
            let avg = total / Double(months)
            print("📊 [Budget Generation]   - \(category): \(amounts.count) transactions, $\(String(format: "%.2f", total)) total, $\(String(format: "%.2f", avg))/month avg")
        }
        ...
        for (category, amounts) in categorySpending {
            ...
            if suggestedLimit > 10 {
                ...
                print("📊 [Budget Generation] ✅ Created budget for \(category): $\(String(format: "%.2f", suggestedLimit))/month")
            } else {
                print("📊 [Budget Generation] ⏭️  Skipped \(category): $\(String(format: "%.2f", suggestedLimit))/month (below $10 threshold)")
            }
        }

        print("📊 [Budget Generation] Final result: Generated \(budgets.count) budgets")
